...
Con la versione corrente del repository, il default locale di `build_deb.sh` produce `yt-transcriber_1.2.0_amd64.deb`.
...
`build_deb.sh` usa `VERSION="1.2.0"` come default locale corrente, ma se la variabile ambiente `APP_VERSION` e' valorizzata usa quel valore dopo la rimozione dell'eventuale prefisso `v`.
