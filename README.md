...
Versione 1.2.5 · 2026 · Licenza: GPL-3.0-or-later
...
In `v1.2.4` il progetto mantiene il backend manager Linux introdotto nella serie `v1.2.x`, con setup guidato per `whisper.cpp` e `faster-whisper`, incluso controllo preliminare `--check-only` per entrambi gli script di setup. Su Linux il setup di `whisper.cpp` tenta la build Vulkan quando le dipendenze sono disponibili e resta con fallback CPU quando non lo sono.
